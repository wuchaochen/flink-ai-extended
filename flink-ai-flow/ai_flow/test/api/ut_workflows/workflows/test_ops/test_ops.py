...
class TestOps(unittest.TestCase):
    ...
    def test_action_with_periodic(self):
        ...
        ops.periodic_run(job_name='task_1',
                         periodic_config=PeriodicConfig(trigger_config={'cron': '* * * * * * *'}))
        self.assertEqual(1, len(current_graph().edges))
        edge: ControlEdge = current_graph().edges.get('task_1')[0]
        self.assertEqual('task_1', edge.destination)
        self.assertEqual('*', edge.condition_config.sender)
        self.assertEqual(AIFlowInternalEventType.PERIODIC_ACTION, edge.condition_config.event_type)
