...
class BaseSchedulerTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.master.start()
        cls.master_port = cls.master.master_config.get_master_port()
        cls.deploy_path = cls.master.master_config.get('scheduler').get('properties').get('airflow_deploy_path')
    ...
    def start_scheduler(cls, file_path, executor=None):
        ...
        scheduler = EventBasedSchedulerJob(
            ...
            server_uri="localhost:{}".format(cls.master_port),
            executor=executor,
            max_runs=-1,
    ...
    def run_ai_flow(self, dag_id, test_function: Callable[[NotificationClient], None],
        ...
        def run_test_fun():
            time.sleep(5)
            client = NotificationClient(server_uri="localhost:{}".format(self.master_port),
