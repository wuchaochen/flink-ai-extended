...
from ai_flow_plugins.job_plugins.flink.flink_env import get_flink_env
    ...
    def submit_job(self, job: Job, job_runtime_env: JobRuntimeEnv = None) -> JobHandle:
        ...
        if not flink_job.is_java:
            ...
            if job_config.run_mode == 'local':
                ...
                if job_config.parallelism is not None:
                    bash_command.extend(['-p', job_config.parallelism])
                bash_command.extend(['-pyexec', python3_location,
                                     '-pyfs'])
        ...
        if flink_job.is_java:
            ...
            while True:
                ...
                num += 1
                if 0 == num % 20:
                    print("Waiting for stdout log file created...")
            while True:
                print(stdout_log)
